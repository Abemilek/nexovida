...
namespace WebApi.Controllers
{
    ...
    {
        private readonly IFamiliarService _service;

        public FamiliarController(IFamiliarService service)
        {
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            if (!resultado.Any())
            {
                return Ok(new { message = "No hay registros de Familiar actualmente." });
            }
            return Ok(resultado);
        }
        ...
        [Authorize(Roles = "Administrador,ProfesionalSalud")]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] FamiliarCreateDto dto)
        {
            ...
            return CreatedAtAction(nameof(GetById), new { id = creado.IdFamiliar }, creado);
        }

        [Authorize(Roles = "Administrador,ProfesionalSalud")]
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] FamiliarUpdateDto dto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(new { message = "La solicitud contiene datos invalidos." });
