...
namespace WebApi.Controllers
{
    ...
    public class EnfermedadController : ControllerBase
    {
        ...
        public EnfermedadController(IEnfermedadService service)
        {
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            if (!resultado.Any())
            {
                return Ok(new { message = "No hay registros de Enfermedad actualmente." });
            }
            return Ok(resultado);
        }
        ...
        [Authorize(Roles = "Administrador,ProfesionalSalud")]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] EnfermedadCreateDto dto)
        {
            ...
            var entity = new Enfermedad
            {
                ...
                EsCronica = dto.EsCronica
            };

            var creado = await _service.CreateAsync(entity);
            return CreatedAtAction(nameof(GetById), new { id = creado.IdEnfermedad }, creado);
        }

        [Authorize(Roles = "Administrador,ProfesionalSalud")]
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] EnfermedadUpdateDto dto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(new { message = "La solicitud contiene datos invalidos." });
