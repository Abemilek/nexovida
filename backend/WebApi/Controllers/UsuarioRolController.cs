...
namespace WebApi.Controllers
{
    ...
    public class UsuarioRolController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            if (!resultado.Any())
            {
                return Ok(new { message = "No hay registros de UsuarioRol actualmente." });
            }
            return Ok(resultado);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] UsuarioRolCreateDto dto)
        {
            ...
            return CreatedAtAction(nameof(GetById), new { id = creado.IdUsuarioRol }, creado);
        }

        [Authorize(Roles = "Administrador")]
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] UsuarioRolUpdateDto dto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(new { message = "La solicitud contiene datos invalidos." });
